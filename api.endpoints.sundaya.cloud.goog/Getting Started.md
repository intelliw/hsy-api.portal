...
The `/devices` path is for receiving monitoring data from trackable devices such as _PMS_, _MPPT_, and _Inverters_. These devices continuously produce data which is then collected and uploaded by device controllers (e.g. Bus Bar Controller) and gateways (e.g. EHub Gateway)through this API path.
