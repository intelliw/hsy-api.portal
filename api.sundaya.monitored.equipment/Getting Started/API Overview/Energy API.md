...
- `buy.in` and `buy.out` mains use, and feed-in/out flows from the public grid.
...
Sources | Sinks    
--- |---
`harvest` `store.out` `buy.out` |`yield` `store.in` `buy.in`
...
- _Black_ in the top tier shows energy drawn from the grid (`buy.out`).
...
- _Black_ in the bottom tier shows a net excess (of `harvest` energy, compared to `store.in` and `yield` energy volumes), resulting in feed-in flows to the grid (`buy.in`).
...
- In the 2nd hour half came from battery and the other half from grid (`buy.out`). 
