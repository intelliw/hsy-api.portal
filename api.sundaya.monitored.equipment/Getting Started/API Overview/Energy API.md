...
- `grid.in` and `grid.out` mains use, and feed-in/out flows from the public grid.
...
### Path parameters
- __energy__ - Specifies one of the four energy data types (described above), or `hsy` if all four data types are required. `hsy` stands for `harvest`, `store`, `yield` but implicitly also includes `grid`.
