...
Energy                      | Description
---                         | --- 
`store.in` `store.out`   | *charge* and *discharge* flows to and from storage batteries.<br>The `battery.amps` data ([mppt](/docs/api.sundaya.monitored.equipment/0/c/Examples/POST/mppt%20POST%20example)) is positive for charge current (`store.in`) and negative when discharging (`store.out`).
`sell` `buy`             | *feed-in* and *feed-out* flows to and from the public or private grid.<br>The `grid.amps` data ([inverter](/docs/api.sundaya.monitored.equipment/0/c/Examples/POST/inverter%20POST%20example)) is positive for feeds into the grid (`sell`) and negative for feeds out of the  grid (`buy`).  
`harvest`                   | renewable energy generated by solar panels and other sources.<br>The `pv.amps` [mppt](/docs/api.sundaya.monitored.equipment/0/c/Examples/POST/mppt%20POST%20example) data is always positive.
`yield`                     | energy used by end users and appliances.<br>The `load.amps` data [mppt](/docs/api.sundaya.monitored.equipment/0/c/Examples/POST/mppt%20POST%20example) is always negative.
