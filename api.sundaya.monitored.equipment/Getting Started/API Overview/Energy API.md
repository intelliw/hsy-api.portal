...
Energy                      | Description
---                         | --- 
`store.in`<br>`store.out`   | *charge* and *discharge* flows to and from storage batteries.<br>`battery.amps` data is positive for charge current and negative when discharging.
`sell`<br>`buy`             | *feed-in* and *feed-out* flows to and from the public or private grid.<br>The `batter.amps` data is positive for charge current and negative when discharging.  
