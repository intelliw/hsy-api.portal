...
- /energy/{`energy`}/period/{`period`}/{`epoch`}/{`duration`}?site={`site`}
    ...
    e.g. [http://api.endpoints.sundaya.cloud.goog/energy/hsy/period/week/20150204/1?site=999](http://api.endpoints.sundaya.cloud.goog/energy/hsy/period/week/20150204/1?site=999 "energy=hsy, period=week, duration=1, site=999")
...
- **self**	- Identifies the link's context.
    ...
    e.g. href=[http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/week/20190210](http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/week/20190210)

    In `collection.items.links` it points to a child item in the collection (`name`=*'day'*).

    e.g. href=[http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/day/20190204](http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/day/20190204)
...
- **item** - in `collection.items.links` it points to the subitems of the child item: i.e the grandchild items of the collection (`name`=*'day.hour'*).
    ...
    e.g. href=[http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/hour/20190205T0600](http:/api.endpoints.sundaya.cloud.goog/energy/hsy/period/hour/20190205T0600)
